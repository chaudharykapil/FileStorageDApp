...
const Level3 = () => {
    ...
    const pass1 = window.localStorage.getItem("login_temp_password1")
    const pass2 = window.localStorage.getItem("login_temp_password2")
    ...
    const signin = ()=>{
      console.log({pass1,pass2,password})
      const signer = contract.connect(provider.getSigner());
      console.log(signer)
      const res = signer.authenticate(pass1,pass2,password);
      res.then(v=>{
        console.log(v)
        if(v){
  ...
  return (
    <>
      ...
      <div className="file-container" style={{height:"100vh",width:"100vw",display:"flex",rowGap:"2rem",flexDirection:"column",justifyContent:"center", alignItems:"center"}}>
        ...
        <div>
            <input type="button" onClick={signin} style={{backgroundColor:"#8707ff",padding:"10px 20px",border:"none",borderColor:"#8707ff", borderRadius:"10px"}} value={"Signin"}/>
        </div>
      </div>
    </>
  );
